...
"""Home screen: main assignment list with stats, charts, and sidebar."""
...
from ..screens.modals import ConfirmPath, InputPrompt, LoadingScreen
from ..state import StateManager
from ..theme import ThemeColors, set_theme
...
class HomeScreen(Screen):
    """Main assignment list screen with stats, charts, and sidebar."""
    ...
    def on_mount(self) -> None:
        ...
        try:
            cached_items = self.state.get_cached_items()
            cached_ann = self.state.get_cached_announcements()
            if cached_items:
                self.items = [CanvasItem.from_dict(d) for d in cached_items]
                has_cached_view = True
            if cached_ann:
                self.announcements = [CanvasItem.from_dict(d) for d in cached_ann]
            if has_cached_view:
                self._render_info()
                self._render_table()
                self._render_stats()
        ...
        self.call_later(lambda: self._initial_load(show_loading=not has_cached_view))
    ...
    def refresh_data(self, silent: bool = False) -> None:
        ...
        if not silent:
            self.details.update("[dim]Refreshing core data…[/dim]")

        def worker() -> None:
            try:
                course_cache, course_scores = self.api.fetch_course_snapshot()
                ...
                self._hydrate_grades_async(list(course_cache.keys()), seq=seq, silent=silent)
        ...
        threading.Thread(target=worker, daemon=True, name="canvas-refresh-core").start()

    def _hydrate_grades_async(self, course_ids: list[int], *, seq: int, silent: bool) -> None:
        def worker() -> None:
            grade_data: dict[int, list[dict[str, Any]]] = {}
            try:
                max_workers = max(2, min(8, len(course_ids) or 1))
                if course_ids:
                    with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="grades") as ex:
                        fut_map = {ex.submit(self.api.fetch_grades, cid): cid for cid in course_ids}
                        for fut in as_completed(fut_map):
                            cid = fut_map[fut]
                            with contextlib.suppress(Exception):
                                grade_data[cid] = fut.result()

                def apply_grades_ui() -> None:
                    if seq != self._refresh_seq:
                        return
                    ...
                    if not silent:
                        self.details.update(
                            "[dim]Live data synced. Select item: Enter (full) or d (quick)\n"
                            "A=announcements S=syllabi G=grades[/dim]"
                        )
                    self._update_status_bar("Grades ready")

                self.call_from_thread(apply_grades_ui)
            except Exception as exc:
                self._error_count += 1
                err_msg = str(exc)
                ...
                def mark_failed() -> None:
                    if seq == self._refresh_seq:
                        self._grade_hydrating = False
                        self._update_status_bar(f"Grade hydrate error: {err_msg}")

                self.call_from_thread(mark_failed)

        threading.Thread(target=worker, daemon=True, name="canvas-refresh-grades").start()
    ...
    def action_open_settings(self) -> None:
        def _on_dismiss(result: dict | None) -> None:
            ...
            self.app.dark = candidate.theme == "dark"
            self._apply_config_layout()
        ...
        self.app.push_screen(SettingsScreen(self), callback=_on_dismiss)
    ...
    def action_open_details(self) -> None:
        ...
        self.app.push_screen(DetailsScreen(self, it))
    ...
    def action_open_grades(self) -> None:
        ...
        self.app.push_screen(GradesScreen(self, self.course_cache))
    ...
    def action_toggle_theme(self) -> None:
        if self._theme.name == "dark":
            self._theme = set_theme("light")
            self.app.dark = False
        else:
            self._theme = set_theme("dark")
            self.app.dark = True
        self._render_info()
        self._render_stats()
        self._render_graphs()
    ...
    def action_open_syllabi(self) -> None:
        if not self.course_cache:
            ...
            return
        from ..screens.syllabi import SyllabiScreen

        self.app.push_screen(SyllabiScreen(self, self.course_cache))
    ...
    def action_open_announcements(self) -> None:
        ...
        self.app.push_screen(AnnouncementsScreen(self, self.announcements))

    def action_open_files(self) -> None:
        if not self.course_cache:
            self.details.update("[yellow]No courses cached yet — refresh first[/yellow]")
            return
        from ..screens.files import FileManagerScreen

        self.app.push_screen(FileManagerScreen(self, self.course_cache))

    def action_open_week(self) -> None:
        from ..screens.weekview import WeekViewScreen

        self.app.push_screen(WeekViewScreen(self, self.items))

    def action_open_dashboard(self) -> None:
        from ..screens.dashboard import DashboardScreen

        self.app.push_screen(DashboardScreen(self))

    def action_open_analytics(self) -> None:
        from ..screens.analytics import AnalyticsScreen

        self.app.push_screen(AnalyticsScreen(self))

    def action_open_rmp(self) -> None:
        from ..screens.rmp import RMPScreen

        self.app.push_screen(RMPScreen())
    ...
    def action_manage_courses(self) -> None:
        ...
        from ..screens.courses import CourseManagerScreen

        self.app.push_screen(CourseManagerScreen(self), callback=_on_dismiss)

...
def anonymize_pairs(pairs: list[dict], output_path: str) -> list[dict]:
    ...
    with open(str(out) + ".course_map.json", "w") as f:
        json.dump({str(k): v for k, v in course_map.items()}, f, indent=2)
    print("[ANON] Wrote: " + str(out) + " + " + str(out) + ".course_map.json")
    print("[ANON] NOTE: Do NOT commit the .course_map.json file — it maps real course IDs!")
    return anon_pairs

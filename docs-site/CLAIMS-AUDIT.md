...
### Plan
- **Model:** Gemma 4B fine-tuned via LoRA on Brev cloud GPU (RTX PRO 6000 or L40S)
...
- [ ] Run LoRA fine-tune on Gemma 4B (see `scripts/finetune_reranker.py`)
...
- Model: google/gemma-4b-it (4-bit quantized via bitsandbytes)
...
4. **Reranker fine-tune** — spin up Brev GPU, run LoRA fine-tune on Gemma 4B
